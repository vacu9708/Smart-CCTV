...
import imagezmq
class MyStream:
    def __init__(self, img_size=640, stride=32, auto=True, raspberry=False):
        self.img_size = img_size
        self.stride = stride
        self.auto = auto
        self.imgs=[None]
        if raspberry:
            self.image_hub = imagezmq.ImageHub() # Guarantee first frame
            rpi_name, self.imgs[0] = self.image_hub.recv_image()
            self.image_hub.send_reply(b'OK')
            self.thread = Thread(target=self.raspberry_update, daemon=True)
        else:
